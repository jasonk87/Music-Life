...
class Event:
    ...
    def __init__(self, name, location, event_type="OPEN_MIC", required_skills=None,
                 ...
                 is_tour_gig=False, is_player_organized=False, required_fame=0, is_npc_gig=False, required_traits=None): # Added is_player_organized, required_traits
        self.required_traits = required_traits if required_traits else []
        self.name = name
...
if player.fame < self.required_fame:
        ...
        if self.required_traits:
            for trait_id in self.required_traits:
                if not player.has_trait(trait_id):
                    return False, f"Player does not have the required trait: {trait_id}", False
