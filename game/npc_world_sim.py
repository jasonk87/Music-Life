...
class NPCWorldSimulator:
    ...
    def _maybe_emit_player_situation(self, npc):
        ...
        player_visibility = self.game.visibility_system.get_visibility(player.name, player_city).get("public_visibility", 0.0)
        npc_visibility = self.game.visibility_system.get_visibility(npc.npc_id, npc_city).get("public_visibility", 0.0)
        player_rep_bias = {"trust": 0.0, "scrutiny": 0.0, "opportunity": 0.0}
        npc_rep_bias = {"trust": 0.0, "scrutiny": 0.0, "opportunity": 0.0}
        if hasattr(self.game, "reputation_system"):
            player_rep_bias = self.game.reputation_system.bias_for(player.name, location=player_city)
            npc_rep_bias = self.game.reputation_system.bias_for(npc.npc_id, location=npc_city)
        ...
        allow_replacement = flow.allow_pressure("opportunity", base_weight=0.85, major=False) if flow else True
        replacement_reliability_gate = -6 - int(player_rep_bias.get("trust", 0.0) * 2)
        if missed_recent and shared_city and allow_replacement and replacement_key not in player.active_opportunities and player_reliability >= replacement_reliability_gate:
            ...
            if flow:
                flow.record_pressure("opportunity", 0.7)

        collab_key = f"collab_offer_{npc.npc_id}"
        allow_collab = flow.allow_pressure("collab", base_weight=0.95, major=False) if flow else True
        collab_respect_gate = max(2, 5 - int(player_rep_bias.get("trust", 0.0)))
        collab_trust_gate = max(1, 3 - int(player_rep_bias.get("trust", 0.0)))
        if shared_city and rel.respect >= collab_respect_gate and rel.trust >= collab_trust_gate and npc.momentum >= 1 and allow_collab and collab_key not in player.active_opportunities and player_reliability >= -2 and (player_visibility + npc_visibility + player_rep_bias.get("opportunity", 0.0)) >= 2:
            ...
            if flow:
                flow.record_pressure("collab", 0.8)

        rivalry_key = f"rivalry_{npc.npc_id}"
        allow_rivalry = flow.allow_pressure("rivalry", base_weight=1.2, major=True) if flow else True
        rivalry_gate = 8 - int(npc_rep_bias.get("controversy", npc_rep_bias.get("scrutiny", 0.0)))
        if rel.rivalry >= max(5, rivalry_gate) and npc.momentum >= 3 and allow_rivalry and rivalry_key not in player.active_opportunities and (player_visibility + npc_visibility) >= 1:
            ...
            if flow:
                flow.record_pressure("rivalry", 1.1)
