...
class UISignalLayer:
    ...
    def get_world_feed(self, limit: int = 6) -> List[Dict[str, str]]:
        ...
        for item in self._opportunity_feed_items(limit=limit - len(feed)):
            ...
            if len(feed) >= limit:
                break
        for item in self._reputation_feed_items(limit=limit - len(feed)):
            feed.append(item)
            if len(feed) >= limit:
                break
    ...
    def _reputation_feed_items(self, limit: int) -> List[Dict[str, str]]:
        if limit <= 0 or not hasattr(self.game, "reputation_system") or not self.game.player:
            return []
        player = self.game.player
        city = player.current_location.name if player.current_location else None
        tags = self.game.reputation_system.summarize(player.name, scope=f"city:{city}" if city else "global", limit=2)
        if not tags:
            return []
        return [{"type": "reputation", "text": f"Scene perception leans: {', '.join(tags)}."}]
    ...
    def _memory_to_world_text(self, entry, player_name: str) -> Optional[str]:
        event_map = {
            ...
            "assistant_secured_booking": "Assistant secured your lodging.",
            "assistant_low_quality_booking": "Assistant found a place, but not your first pick.",
            ...
            "security_filtered_crowd": "Security filtered the crowd around you.",
            "manager_overbooked_pressure": "Manager shuffled plans at the last minute.",
            "manager_secured_strong_opportunity": "Manager lined up a strong opportunity.",
            "hotel_booked": "You got a room booked.",
            "hotel_downgraded": "You got a room, but below your preferred tier.",
            "hotel_unavailable": "No rooms were available in your target range.",
            "public_interference_spike": "Public attention made movement around town harder.",
            "contract_signed": "A new contract was signed.",
            "contract_pressure_generated": "Organization deadlines are stacking up.",
            "contract_warning": "Organization review pressure is rising.",
            "contract_second_chance": "You were given another shot.",
            "contract_dropped": "The organization cut ties.",
            "project_started": "A new creative project started.",
            "studio_session_completed": "A studio session moved your project forward.",
            "rushed_session": "A rushed session left the work uneven.",
            "collaboration_confirmed": "A collaboration was confirmed.",
            "single_released": "A new single dropped.",
            "album_released": "A new album released.",
            "video_shot": "A music video was completed.",
            "breakout_release": "The release is gaining strong traction.",
            "poor_release_reception": "The release landed with weak response.",
            "promo_event_completed": "Promo activity is in motion.",
        }
    ...
    def _memory_to_player_text(self, entry) -> Optional[str]:
        event_map = {
            ...
            "assistant_secured_booking": "Your assistant secured your lodging.",
            "assistant_low_quality_booking": "Your assistant booked a lower-fit room.",
            ...
            "manager_secured_strong_opportunity": "Your manager lined up a strong opportunity.",
            "hotel_booked": "You booked a room.",
            "hotel_downgraded": "You booked a room below your preferred tier.",
            "hotel_unavailable": "You couldn't find a room in your target range.",
            "public_interference_spike": "Public interference slowed you down.",
            "contract_signed": "You signed a contract.",
            "contract_pressure_generated": "Contract pressure added new work to your calendar.",
            "contract_warning": "You received a contract warning.",
            "contract_second_chance": "You got a second chance under contract.",
            "contract_dropped": "Your contract was dropped.",
            "project_started": "You started a new creative project.",
            "studio_session_completed": "You completed a productive studio session.",
            "rushed_session": "You rushed a session and quality slipped.",
            "collaboration_confirmed": "You locked in a collaboration.",
            "single_released": "You released a single.",
            "album_released": "You released an album.",
            "video_shot": "You completed a music video.",
            "breakout_release": "One of your releases is breaking out.",
            "poor_release_reception": "A release underperformed.",
            "promo_event_completed": "You completed promo-related work.",
        }
        return event_map.get(entry.event_type)
