...
class SocialMediaExpressionLayer:
    ...
    def _template_render(self, entry, voice_profile: PersonalityProfile) -> str:
        ...
        if voice_profile.archetype == "friendly":
            ...
            return fan_mapping.get(entry.event_type, neutral_mapping.get(entry.event_type, "People are talking."))
        if voice_profile.archetype == "arrogant":
            ...
            return rival_mapping.get(entry.event_type, neutral_mapping.get(entry.event_type, "Scene chatter is active."))
        return neutral_mapping.get(entry.event_type, "Scene chatter is active.")
