...
class ObligationResolver:
    ...
    def _evaluate_obligation(self, item, now: GameTime) -> ObligationResolution:
        ...
        adjusted_minutes = option.minutes
        if hasattr(self.game, "delegation_system") and hasattr(self.game, "world_memory"):
            location_ref = getattr(destination, "parent_location_id", getattr(destination, "name", None))
            adjusted_minutes, _, _ = self.game.delegation_system.driver_adjust_trip(
                self.game.player,
                option.minutes,
                world_memory=self.game.world_memory,
                location_ref=location_ref,
            )
            option = TravelOption(mode=option.mode, minutes=adjusted_minutes, cost=option.cost)
