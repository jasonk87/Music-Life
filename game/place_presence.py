...
class ContextualEncounterEngine:
    def maybe_trigger(self, player, profile: PlaceProfile, action: LocalAction, rng: Optional[random.Random] = None) -> Optional[EncounterResult]:
        rng = rng or random
...
class LocationActionEngine:
    ...
    def execute_action(
        ...
        encounter = self.encounters.maybe_trigger(player, profile, action, rng=rng)
